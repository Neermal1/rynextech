...
const TopFeature = ({ feature_data }: ITopFeature) => {
  const featureReference = useRef<HTMLDivElement | null>(null);
  const [isClient, setIsClient] = useState(false);

  useEffect(() => {
    setIsClient(true);

    if (typeof window !== "undefined" && featureReference.current) {
  ...
  return (
    <div className="layout component-padding">
      ...
      >
        <motion.div className="lg:col-span-5 rounded-md">
          <div className="relative">
            {isClient && <div ref={featureReference}></div>}
          </div>
        </motion.div>

        <motion.div className="lg:col-span-7 flex flex-col gap-10">
